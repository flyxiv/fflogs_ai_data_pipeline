...
class NinjaEnvironment:
	...
	def _get_state(self):
		resources_normalized = [self.resources.cooldowns[i] / MAX_COOLDOWN_MILLISECOND for i in range(len(self.resources.cooldowns))] 
		stacks_normalized = [self.resources.stacks[NinjaStacks.NINKI.value] / 100, self.resources.stacks[NinjaStacks.SHURIKEN.value] / 5, self.resources.stacks[NinjaStacks.MUDRA.value] / 2, self.resources.stacks[NinjaStacks.TCJ_FUMA.value], self.resources.stacks[NinjaStacks.TCJ_RAITON.value]]
		status_normalized = sum([[self.resources.status[i][0] / MAX_STATUS_DURATION_MILLISECOND, self.resources.stacks[i][1] / MAX_STACKS] for i in range(len(self.resources.stacks))], list()) 
		combo_normalized = [self.resources.combo / 3, self.resources.combo_timer / COMBO_TIMER_MAX_MILLISECOND]

		return np.array(resources_normalized + stacks_normalized + status_normalized + combo_normalized)
	...
	def step(self, action, time_elapsed_millisecond):
		...
		if action >= len(self.resources.cooldowns):
			if self.resources.cooldowns[action] > 0:
				reward = -50
			elif not self._has_requirements_for_skill(action):
				reward = -50
			else:
	...
	def _advance_time(self, time_elapsed_millisecond):
		for skill in self.resources.cooldowns.keys():
			if self.resources.cooldowns[skill]:
				self.resources.cooldowns[skill] = max(self.resources.cooldowns[skill] - time_elapsed_millisecond, 0)
		
		for status_key, value in self.resources.status.items():
			self.resources.status[status_key][0] = value - time_elapsed_millisecond
		...
		for debuff_key, value in self.resources.debuffs.items():
			self.resources.debuffs[debuff_key][0] = value - time_elapsed_millisecond
		...
		self.resources.mudra_timer = max(0, self.resources.mudra_timer - time_elapsed_millisecond)
	...
	def _has_requirements_for_skill(self, action):
		if action == NinjaSkills.BHAVACAKRA.value: 
			return self.resources.stacks[NinjaStacks.NINKI.value] >= 50
		if action == NinjaSkills.ZESHO_MEPPO.value:
			return (self.resources.stacks[NinjaStacks.NINKI.value] >= 50) and (self.resources.status[NinjaStatus.HIGI_STATUS.value])
		elif action == NinjaSkills.RAITON.value or action == NinjaSkills.SUITON.value:
	...
	def _update_state(self, action):
		...
		if action == NinjaSkills.AEOLIAN_EDGE.value:
			if self.resources.combo == 2:
				self.resources.stacks[NinjaStacks.NINKI.value] = min(100, self.resources.stacks[NinjaStacks.NINKI.value] + 15)
		
			self.resources.stacks[NinjaStacks.SHURIKEN.value] = max(self.resources.stacks[NinjaStacks.SHURIKEN.value] - 1, 0)

			if self.resources.status[NinjaStatus.BUNSHIN_STATUS.value]:
				assert self.resources.status[NinjaStatus.BUNSHIN_STATUS.value][1] >= 1, "bunshin status didn't disappear when stack is 0"
			...
			self.resources.stacks[NinjaStacks.SHURIKEN.value] = min(self.resources.stacks[NinjaStacks.SHURIKEN.value] + 2, 5)
			self.resources.stacks[NinjaStacks.NINKI.value] = min(100, self.resources.stacks[NinjaStacks.NINKI.value] + 15)
			...
			assert self.resources.stacks[NinjaStacks.NINKI.value] >= 50, f'using bhavacakra @ _update_state, but ninki={self.resources.stacks[NinjaStacks.NINKI.value]} < 50'
			...
			self.resources.status[NinjaStatus.BUNSHIN_STATUS.value] = [30000, 5]
			self.resources.status[NinjaStatus.PHANTOM_KAMAITACHI_READY.value] = [45000, 1]
			...
			self.resources.stacks[NinjaStacks.NINKI.value] = min(100, self.resources.stacks[NinjaStacks.NINKI.value] + 40)
			self.resources.cooldowns[NinjaSkills.DOKUMORI.value] = NINJA_SKILL_COOLDOWN_MILLISECONDS[NinjaSkills.DOKUMORI.value] 
			self.resources.debuffs[NinjaDebuffs.DOKUMORI_DEBUFF.value] = [21000, 1]
			...
			self.resources.debuffs[NinjaDebuffs.KUNAIS_BANE_DEBUFF.value] = [16250, 1]
			...
			self.resources.status[NinjaStatus.KASSATSU_STATUS.value] = [15000, 1]
			...
			self.resources.status[NinjaStatus.MEDICATED_STATUS.value] = [30000, 1]
			...
			self.resources.stacks[NinjaStacks.NINKI.value] = min(100, self.resources.stacks[NinjaStacks.NINKI.value] + 50)
			...
			self.resources.stacks[NinjaStacks.NINKI.value] = min(100, self.resources.stacks[NinjaStacks.NINKI.value] + 10)
			...
			self.resources.status[NinjaStatus.TENRI_JINDO_READY.value] = [30000, 1]
		elif action == NinjaSkills.ZESHO_MEPPO.value:
			...
			assert self.resources.stacks[NinjaStacks.NINKI.value] >= 50, f'using zesho meppo @ _update_state, but ninki={self.resources.stacks[NinjaStacks.NINKI.value]} < 50'
			self.resources.stacks[NinjaStacks.NINKI.value] = self.resources.stacks[NinjaStacks.NINKI.value] - 50
