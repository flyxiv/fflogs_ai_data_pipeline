...
class SumTreeTest(unittest.TestCase):
    ...
    def test_sum_tree_sample(self):
        ...
        for i, priority in enumerate(priorities):
            ...
            assert (
                probabilities[i] - self.SAMPLE_ERROR_BOUND
                <= sample_ratio
                <= probabilities[i] + self.SAMPLE_ERROR_BOUND
            ), f"sample_ratio for item {i} should be {probabilities[i]}, but it is {sample_ratio}"
