...
from rotation_models.const import COMBO_TIMER_MAX_MILLISECOND, UNIT_TIME_PER_ACTION_MILLISECOND, MAX_POTENCY
...
class CombatStatus:
    ...
    def __init__(self, job_database: JobDatabase, skills, resources, target_time_millisecond: int = 0):
        ...
        self.combat_time_millisecond = 0
        self.target_time_millisecond = target_time_millisecond
        self.max_gcd_delay = max([skill.delay_data.gcd_cooldown_millisecond for skill in self.skills if skill.delay_data])
    ...
    def advance_time(self, delta_time_millisecond: int):
        if self.combo_duration_millisecond:
            self.combo_duration_millisecond = max(0, self.combo_duration_millisecond - delta_time_millisecond)
        ...
        self.gcd_cooldown_millisecond = max(0, self.gcd_cooldown_millisecond - delta_time_millisecond)

    def use_skill(self, skill_id: int):
        if skill_id == 0:
            self.advance_time(UNIT_TIME_PER_ACTION_MILLISECOND)
            return self.get_state(), self.get_valid_skills(), self.combat_time_millisecond, 0, self.combat_time_millisecond >= self.target_time_millisecond

        skill = self.skills[skill_id - 1]

        potency, delay_data = skill.use_skill(self)

        self.gcd_cooldown_millisecond = max(delay_data.gcd_cooldown_millisecond, self.gcd_cooldown_millisecond)

        self.advance_time(delay_data.delay_millisecond)

        return self.get_state(), self.get_valid_skills(), self.combat_time_millisecond, potency / MAX_POTENCY, self.combat_time_millisecond >= self.target_time_millisecond
    ...
    def get_state(self):
        ...
        states.extend([self.combo / 3, self.combo_duration_millisecond / COMBO_TIMER_MAX_MILLISECOND if self.combo_duration_millisecond else 10])
