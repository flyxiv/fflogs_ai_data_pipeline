...
class StatusEvent:
    """Implements status events that happen from skill usage
    """
...
class ApplyBuffEvent(StatusEvent):
    def __init__(self, buff_id: int, duration_millisecond: int, stacks=1, refresh_duration: bool = False):
    ...
    def handle_event(self, combat_status):
        if combat_status.buffs[self.buff_id]:
            if self.refresh_duration:
                combat_status.buffs[self.buff_id].duration_millisecond = self.duration_millisecond
...
class ApplyDebuffEvent(StatusEvent):
    def __init__(self, debuff_id: int, duration_millisecond: int, stacks=1, refresh_duration: bool = False):
    ...
    def handle_event(self, combat_status):
        if combat_status.debuffs[self.debuff_id]:
            if self.refresh_duration:
                combat_status.debuffs[self.debuff_id].duration_millisecond = self.duration_millisecond
