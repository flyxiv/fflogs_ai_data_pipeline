...
class NinjaEnvironment:
    ...
    def _get_state(self):
        ...
        for i in range(len(self.resources.debuffs)):
            if self.resources.debuffs[i]:
                debuffs_encoded.append(
                    self.resources.debuffs[i][0] / MAX_STATUS_DURATION_MILLISECOND)
                debuffs_encoded.append(
        ...
        r = tf.expand_dims(tf.constant(np.array(resources_normalized + stacks_normalized +
                           status_encoded + debuffs_encoded + combo_normalized + is_gcd)), axis=0)
        return r
    ...
    def get_possible_gcd_actions(self):
        ...
        for skill_id in range(len(NinjaGCDSkills)):
            ...
            if self._has_requirements_for_skill(skill_id_translated.value) and self.resources.cooldowns[skill_id_translated.value] == 0:
    ...
    def step(self, action_id, time_elapsed_millisecond, debug_mode=False):
        logging.info(f'{self.current_time_millisecond}, action: {action_id}')
        ...
        if action_id <= len(self.resources.cooldowns):
    ...
    def _has_requirements_for_skill(self, skill_id):
        ...
        elif skill_id == NinjaSkills.RAITON.value or skill_id == NinjaSkills.SUITON.value:
